...
public class VipHandler extends Handler {
    ...
    public void handle() {
        ...
        .asyncFirst(() -> LuckPermsHelper.addPlayerToGroup(nick, "vip", (long)months*30*24*60*60))
        .syncLast((result) -> {
            if (result) {
                solve(Result.SUCCESS, "VIP úspěšně aktivováno!", "VIP added");
                TaskChainHelper.newChain()
                .asyncFirst(() -> LuckPermsHelper.getPlayerGroupDuration(nick, "vip"))
                .syncLast((duration) -> {
                    if (duration <= 0) {
